...
def main() -> None:
    ...
    if bundle is None:
        raise SystemExit("Processed tensors are missing. Stopped without changing the architecture.")
    report = Path("experiments/vanilla_bce_training_report.md")
    payload = train_vanilla_bce(config, bundle, report)
